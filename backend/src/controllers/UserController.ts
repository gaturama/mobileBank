...
export const atualizarDadosUsuario = async (req: Request, res: Response) => {
  try {
    ...
    const { nome_completo, email, telefone, data_nascimento, endereco, biometria_ativa, autenticacao_2fa } = req.body

    const atualizado = await User.findByIdAndUpdate(
      userId,
      { nome_completo, email, telefone, data_nascimento, endereco, biometria_ativa, autenticacao_2fa },
      { new: true, runValidators: true, context: 'query' }
    ).select('-senha_hash')

    if (!atualizado) return res.status(404).json({ error: 'Usuário não encontrado' })

    return res.json({ message: 'Dados atualizados com sucesso', usuario: atualizado })
  } catch (error) {
