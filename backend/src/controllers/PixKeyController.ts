...
export const criarPixKey = async (req: Request, res: Response) => {
  try {
    ...
    if (!tipo_chave || !valor_chave) return res.status(400).json({ error: 'Tipo e valor da chave são obrigatórios' })

    const chaveExistente = await PixKey.findOne({ valor_chave })
    if (chaveExistente) return res.status(400).json({ error: 'Chave Pix já cadastrada' })
    ...
    return res.status(201).json({ message: 'Chave Pix criada com sucesso', chave: novaChave })
  } catch (error) {
...
export const atualizarStatusPixKey = async (req: Request, res: Response) => {
  try {
    ...
    if (!userId) return res.status(401).json({ error: 'Usuário não autenticado' })
    if (!['ativa', 'inativa'].includes(status)) return res.status(400).json({ error: 'Status inválido' })

    const chave = await PixKey.findOne({ _id: id, id_conta: userId })
    if (!chave) return res.status(404).json({ error: 'Chave Pix não encontrada' })
